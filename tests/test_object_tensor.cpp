...
int main(){
  ...
  memtier_tensor_desc_t desc{}; desc.path=path; desc.data_offset=1<<20; desc.nbytes=1<<20; desc.dtype=MEMTIER_DTYPE_UINT8;
  ...
  memtier_stats_t s2{}; memtier_stats(ctx,&s2); if(s2.dram_hits<=s1.dram_hits) return 5;
