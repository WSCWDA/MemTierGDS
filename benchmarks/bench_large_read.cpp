...
int main(int argc,char**argv){std::string file="/mnt/gds2/cwd_test/test.bin",mode="memtier";size_t size=1ull<<30;for(int i=1;i<argc;++i){if(std::strncmp(argv[i],"--file=",7)==0)file=argv[i]+7;else if(std::strncmp(argv[i],"--size=",7)==0)size=std::stoull(argv[i]+7);else if(std::strncmp(argv[i],"--mode=",7)==0)mode=argv[i]+7;}
if(!std::filesystem::exists(file)){ if(memtier_prepare_experiment_file("/mnt/gds2/cwd_test", std::filesystem::path(file).filename().c_str(), size)!=0){ std::cerr<<"prepare file failed\n"; return 1; }}
