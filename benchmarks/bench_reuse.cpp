...
int main(int argc,char**argv){const char* file="/mnt/gds2/cwd_test/test.bin"; if(argc>1) file=argv[1];
  if(!std::filesystem::exists(file)){ if(memtier_prepare_experiment_file("/mnt/gds2/cwd_test", std::filesystem::path(file).filename().c_str(), 1ull<<30)!=0){ std::cerr<<"prepare file failed\n"; return 1; }}
  run(file,"posix"); run(file,"gds"); run(file,"memtier"); }
